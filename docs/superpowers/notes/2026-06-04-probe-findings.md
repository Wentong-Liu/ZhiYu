...
- **对策（已落地，实测 198ms）**：只导航到**右侧会话面板** `AXSplitGroup` 读消息列表（约 30–40 条已渲染行），**完全不下钻左侧巨表**；每行只读文本属性；正常读取不再 dump 整树（移到单独诊断按钮）；去掉 wake 整树遍历。

## 关键实现细节（Phase 2 WeChatReader 直接复用）
- **消息文本在 `AXTitle`（其次 `AXDescription`），不在 `AXValue`**：消息气泡是 `AXUnknown` 节点；用 `bestText = AXValue ?? AXTitle ?? AXDescription` 取值。注意**输入框草稿在 `AXValue`**（两者不同）。
- 消息表 `AXChildren` 直接给出 `AXRow/AXTableRow`（实测 42 行/29 行随会话），每行下钻到叶子取 `bestText`。
- **发送两条可用路径**：① AX 写 `AXValue` → `activate()` 微信 + 给 composer 设 `AXFocused` → 等 ~0.4s 并校验"微信在前台/输入框聚焦" → 模拟回车（干净，不动剪贴板，**推荐**）；② 粘贴 + 回车（兜底）。**只有当微信确为前台且输入框聚焦时回车才会发出**——这是关键正确性条件。

## 对 Phase 2 架构的确定性结论
- **读取主力 = 纯 AX**（路线 A 成立）；OCR（路线 C）降级为可选兜底，首版可不做。
- `WeChatReader` 设计：定位主 `AXSplitGroup` → 其直接子 `AXSplitGroup`(右侧面板) → 消息 `AXTable`（取最近 N 行，用 `bestText` 取文本）+ 顶部联系人 `AXStaticText` + 底部 `AXTextArea`(草稿在 `AXValue`/锚定)。
- 说话人解析：`^我说[:：]`/`^我[:：]`→me；`^(.+?)说[:：]`/`^(.+?)[:：]`→other(发言人=捕获)；纯时间行跳过。
- 插入 = AX 写 `AXValue` 优先、粘贴兜底；发送 = **激活微信 + 聚焦 composer + 校验前台/焦点 + 模拟回车**；触发 = 全局快捷键(正式版换 RegisterEventHotKey 独占)。
